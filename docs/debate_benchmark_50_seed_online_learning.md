...
| **Regime 3: True Pinned Pareto** | Loads the 355-step warm Pareto frontier, but freezes mutation during the benchmark. | `--pin-pareto` | Frozen (Warm v355) | Evaluates the stationary, zero-drift benefit of warm Pareto prompt directives without online mutation overhead. |
...
| **Total Attempts** | 73 (1.46/seed) | 67.20 ± 3.27 (1.34/seed) | **74 (1.48/seed)** | **83 (1.66/seed \| 1.46/ep)\*** | **69 (1.38/seed)** | **69 (1.38/seed)** | **75 (1.50/seed)** | **74.00 ± 5.74 (1.48/seed)** | **[66.87, 81.13] attempts** |
...
> [!NOTE]
> **\*Attempt, Episode, and Recovery Accounting**:
> Across the five completed runs, 257 total initial Round 0 evaluation episodes were executed (50 distinct seeds each in Reps 1, 3, 4, 5, plus 57 episodes in Rep 2 due to a pre-restart checkpoint boundary where 7 initial episodes were recorded prior to a complete 50-seed sweep):
> - **Initial R0 Successes:** **144 episodes** were accepted on Round 0 across all runs ($26 + 31 + 31 + 31 + 25 = 144$).
> - **Initial R0 Failures / Refinement Denominator:** **113 episodes** failed on Round 0 and entered Round 1 refinement ($24 + 26 + 19 + 19 + 25 = 113$).
> - **Round 1 Recoveries:** **57 episodes** successfully recovered on Round 1 ($14 + 7 + 8 + 14 + 14 = 57$), establishing an exact **50.4% cumulative recovery rate** ($57 / 113$).
> - **Accepted Training Rows:** $144 \text{ (R0)} + 57 \text{ (R1)} = \mathbf{201 \text{ accepted rows}}$ (yielding $40.20 \pm 2.77$ rows/run, or $80.4\% \pm 5.5\%$).
> - **Attempt Accounting:** $257 \text{ R0 attempts} + 113 \text{ R1 attempts} = \mathbf{370 \text{ total attempts}}$ recorded across the five replicate logs.
...
Across the full five-replicate benchmark suite (50 distinct virgin scenarios across 250 replicate evaluations, totaling 257 evaluation episodes), 113 Round 0 failure episodes triggered refinement. The active reflector triggered graph-informed topological mutations, enabling the swarm to recover and accept **57 of the 113 failing episodes on Round 1 (50.4% cumulative recovery rate)** (14/24 in Rep 1, 7/26 in Rep 2, 8/19 in Rep 3, 14/19 in Rep 4, 14/25 in Rep 5). The live Reflector effectively cut the net failure rate in half across all evaluation episodes.

### C. Evolution of the Pareto Frontier (`artifacts/gepa_pareto_50`)
Over the course of the five completed replicates, the Pareto ledger expanded from **355 to 669 mutations** (+314 candidate variants recorded across 370 attempts). The Pareto composite scores across the three highest-volume taxonomy buckets (`concurrency`, `input_validation`, and `memory_safety`) increased monotonically as verified rows accumulated, while `integer_arithmetic` evolved non-monotonically within a narrow range before stabilizing (+29.9% net gain):
- **`concurrency`**: Score improved monotonically from $7.70 \to 9.29 \to 13.16 \to 17.06 \to 18.98 \to \mathbf{22.74}$ (+195.3%)
- **`input_validation`**: Score improved monotonically from $68.92 \to 80.29 \to 99.16 \to 113.48 \to 127.96 \to \mathbf{141.43}$ (+105.2%)
- **`memory_safety`**: Score improved monotonically from $133.35 \to 145.37 \to 167.46 \to 188.32 \to 206.49 \to \mathbf{221.05}$ (+65.8%)
- **`integer_arithmetic`**: Score evolved non-monotonically from $5.12 \to 4.86 \to 5.79 \to 6.75 \to 6.73 \to \mathbf{6.65}$ (+29.9% net gain)
...
| :--- | :---: | :--- | :--- | :--- |
| `unrun-sliced-pareto-50-rep1` | **Completed** | `test_corpus_sliced_pareto_50_rep1.jsonl` | `artifacts/attempts/unrun-sliced-pareto-50-rep1.jsonl` | `artifacts/runs/unrun-sliced-pareto-50-rep1/batch_manifest.json` |
| `unrun-sliced-pareto-50-rep2` | **Completed** | `test_corpus_sliced_pareto_50_rep2.jsonl` | `artifacts/attempts/unrun-sliced-pareto-50-rep2.jsonl` | `artifacts/runs/unrun-sliced-pareto-50-rep2/batch_manifest.json` |
| `unrun-sliced-pareto-50-rep3` | **Completed** | `test_corpus_sliced_pareto_50_rep3.jsonl` | `artifacts/attempts/unrun-sliced-pareto-50-rep3.jsonl` | `artifacts/runs/unrun-sliced-pareto-50-rep3/batch_manifest.json` |
| `unrun-sliced-pareto-50-rep4` | **Completed** | `test_corpus_sliced_pareto_50_rep4.jsonl` | `artifacts/attempts/unrun-sliced-pareto-50-rep4.jsonl` | `artifacts/runs/unrun-sliced-pareto-50-rep4/batch_manifest.json` |
| `unrun-sliced-pareto-50-rep5` | **Completed** | `test_corpus_sliced_pareto_50_rep5.jsonl` | `artifacts/attempts/unrun-sliced-pareto-50-rep5.jsonl` | `artifacts/runs/unrun-sliced-pareto-50-rep5/batch_manifest.json` |
