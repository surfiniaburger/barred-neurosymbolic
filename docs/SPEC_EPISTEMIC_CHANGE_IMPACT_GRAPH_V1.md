...
1. **Reachability Predicate ($R_{\text{reach}}$):**
   $$R_{\text{reach}}(v) \in \lbrace \text{PROVEN\textunderscore REACHABLE}, \text{UNKNOWN\textunderscore REACHABLE}, \text{PROVEN\textunderscore UNREACHABLE} \rbrace$$
   ...
   - Incomplete ASTs, unresolvable pointer aliases, dynamic FFI calls, or unmapped macro expansions likewise produce `UNKNOWN_REACHABLE`, which **fails closed** ($\omega(R_{\text{reach}}) = 1.0$) to avoid prematurely suppressing unverified attack vectors.
2. **Guard Verification Predicate ($\text{GuardVerified}$):**
   $$\text{GuardVerified}(v) \in \lbrace \text{VERIFIED\textunderscore COMPLETE}, \text{CANDIDATE\textunderscore ONLY}, \text{ABSENT} \rbrace$$
...
0.0 & \text{if } R_{\text{reach}}(v) = \text{PROVEN\textunderscore UNREACHABLE} \\
0.0 & \text{else if } \text{GuardVerified}(v) = \text{VERIFIED\textunderscore COMPLETE} \\
...
where $\omega(\text{PROVEN\textunderscore REACHABLE}) = 1.0$, $\omega(\text{UNKNOWN\textunderscore REACHABLE}) = 1.0$, $\omega(\text{PROVEN\textunderscore UNREACHABLE}) = 0.0$, and the asset exposure weights are explicitly: $\gamma(\text{EXPOSED}) = 1.0$, $\gamma(\text{UNKNOWN}) = 1.0$, and $\gamma(\text{ISOLATED}) = 0.0$.
