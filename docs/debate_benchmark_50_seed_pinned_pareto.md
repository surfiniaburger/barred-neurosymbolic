...
1. **Observed Protocol-Level Recovery Gap:** Across the 249 canonical episodes in Regime 3, 150 seeds passed Round 0, leaving **99 canonical failed episodes that triggered Round 1 refinement**. Retrying with the *same frozen prompt* under Pinned Pareto recovered **37 of 99 episodes (37.4% recovery rate)**. In comparison, the dynamic refinement protocol in Regime 2 achieved a **50.4% recovery rate (57/113)**, which counts failed initial Round-0 attempts among 257 initial attempts (including seven pre-restart checkpoint episodes) rather than solely the 250 canonical evaluations. While differing starting prompt configurations and episode sets preclude attributing the +13.0 pp gap solely to runtime adaptation, this demonstrates a substantial protocol-level recovery difference between static retries and active online adaptation.
